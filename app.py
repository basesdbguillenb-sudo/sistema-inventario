...
COLOR_HEADER_FILL = (219, 229, 241)  # azul claro tipo Office

def generar_numero_reporte(tecnico_nombre, mid):
    tecnico_limpio = re.sub(r'[^A-Za-zÁÉÍÓÚÑáéíóúñ ]', '', str(tecnico_nombre or '')).strip()
    primera_palabra = tecnico_limpio.split(" ")[0] if tecnico_limpio else "GEN"
    letras = (primera_palabra.upper() + "XXX")[:3] if len(primera_palabra) < 3 else primera_palabra[:3].upper()
    try:
        res_count = supabase.table("mantenimientos").select("id", count="exact").ilike("tecnico", f"{primera_palabra}%").lte("id", mid).execute()
        numero = res_count.count if res_count.count else 1
    except Exception:
        numero = 1
    return f"MNT-{letras}-{numero:04d}"
